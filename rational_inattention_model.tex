...
The principal's sensitivity~\eqref{eq:H-def} then simplifies to $H(p) = u''(d^*(p),p) = -G_0''(p)$, so $|H(p)| = G_0''(p)$. Write $B(p) \equiv G_0''(p)$ for the curvature of the principal's own scoring rule.

A natural benchmark is the \emph{pass-through rule}: the principal assigns the sub-agent a scaled version of her own rule $S_0$. Scaling $S_0$ by $a > 0$ scales the curvature by $a$, so the pass-through rule has $g(p) = a\,B(p)$ for some $a > 0$ chosen optimally. We compare the principal's net utility under this rule against the pointwise-optimal curvature~\eqref{eq:ri-optimal-curvature}.
...
\end{equation}
where we used $\int f = 1$. To verify that $\eta \leq 1$ (i.e., the pass-through rule is weakly worse than the optimum), we need to show:
\[
\left(\int_0^1 \sqrt{B\,f\,\phi}\;dp\right)^2 \leq \int_0^1 B\,\phi\,dp.
\]
This follows from the Cauchy--Schwarz inequality: writing $\int \sqrt{B\,f\,\phi}\,dp = \int \sqrt{f} \cdot \sqrt{B\,\phi}\,dp$ and applying $\int uv \leq \sqrt{\int u^2} \cdot \sqrt{\int v^2}$ with $u = \sqrt{f}$ and $v = \sqrt{B\,\phi}$ gives $\int \sqrt{B\,f\,\phi}\,dp \leq \sqrt{\int f\,dp} \cdot \sqrt{\int B\,\phi\,dp} = \sqrt{\int B\,\phi\,dp}$. Squaring yields the desired inequality. Equality holds if and only if $\sqrt{f} \propto \sqrt{B\,\phi}$, i.e., $B(p)\,\phi(p)/f(p) = \text{const}$, which is exactly the condition~\eqref{eq:ri-Su-condition} for $S_u$ to be optimal.
