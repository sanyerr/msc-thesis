...
We now show that for small information cost $c$, the optimal reporting distribution $\pi^*(\hat{p} \mid p)$ is well approximated by a Gaussian. The argument proceeds in three steps: (i) a Taylor expansion of the expected score $V_p$ around its maximum, (ii) control of the resulting approximation error on the full domain $[0,1]$, and (iii) moment computation under the approximate density.

\medskip

\textbf{Step 1: Taylor expansion of $V_p$.}
Recall that every strictly proper scoring rule admits a representation in terms of a convex function $G$ (the \emph{entropy function} or \emph{expected score function}):
\[
...
\[
V_p'(\hat{p}) = G'(\hat{p}) + \frac{d}{d\hat{p}}\!\big[G'(\hat{p})(p - \hat{p})\big] = G''(\hat{p})(p - \hat{p}),
\]
which confirms $V_p'(p) = 0$. Differentiating once more:
\[
V_p''(\hat{p}) = G'''(\hat{p})(p - \hat{p}) - G''(\hat{p}).
\]
Evaluating at $\hat{p} = p$, the first term vanishes and we obtain $V_p''(p) = -G''(p)$. Since $V_p$ has a strict maximum at $p$, we have $G''(p) > 0$.

By Taylor's theorem with integral remainder, for $\delta = \hat{p} - p$:
\begin{equation}\label{eq:taylor-vp}
V_p(p + \delta) = V_p(p) - \tfrac{1}{2}\,G''(p)\,\delta^2 + E(\delta),
\end{equation}
where the remainder satisfies
\[
E(\delta) = \int_0^1 (1-t)\big[V_p''(p + t\delta) - V_p''(p)\big]\,\delta^2\,dt.
\]

We now state the approximation result precisely.

\begin{proposition}[Gaussian approximation]\label{prop:gaussian-approx}
Let $S$ be a strictly proper scoring rule whose entropy function $G$ is $C^3$ on $(0,1)$, and let $p \in (0,1)$. Assume:
\begin{enumerate}
\item[\textup{(A1)}] $G''(p) > 0$;
\item[\textup{(A2)}] $G'''$ is bounded on a neighborhood of $p$: there exist $\eta > 0$ and $M < \infty$ such that $|G'''(\hat{p})| \leq M$ for all $\hat{p} \in (p - \eta, p + \eta) \cap (0,1)$;
\item[\textup{(A3)}] The endogenous marginal $q$ is positive and $C^2$ on a neighborhood of $p$.
\end{enumerate}
Then as $c \to 0^+$:
\begin{align}
\mathbb{E}[\hat{p} - p \mid p] &= \frac{q'(p)}{q(p)} \cdot \frac{c}{G''(p)} + O(c^{3/2}), \label{eq:mean-expansion}\\
\operatorname{Var}(\hat{p} - p \mid p) &= \frac{c}{G''(p)} + O(c^{3/2}). \label{eq:var-expansion}
\end{align}
Moreover, the total variation distance between $\pi^*(\cdot \mid p)$ and the Gaussian $N\!\big(0,\, c/G''(p)\big)$ is $O(\sqrt{c})$ as $c \to 0^+$.
\end{proposition}

\begin{proof}
Write $a = G''(p)/(2c)$ and $\sigma^2 = c/G''(p) = 1/(2a)$.

\medskip
\textit{Step 2: Domain truncation and tail control.}
Define the truncated domain $I_c = [p - c^{1/3}, p + c^{1/3}] \cap [0,1]$. We show that the mass outside $I_c$ is exponentially negligible.

By assumption (A2), for $|\delta| \leq \eta$ the remainder in~\eqref{eq:taylor-vp} satisfies $|E(\delta)| \leq M|\delta|^3/6$. For $|\delta| > \eta$, the strict properness of $S$ guarantees $V_p(p+\delta) < V_p(p)$; moreover, by continuity and compactness of $[0,1]$, we have $V_p(p+\delta) \leq V_p(p) - r$ for some $r = r(\eta, p) > 0$ whenever $|\delta| \geq \eta$.

For $c$ small enough that $c^{1/3} < \eta$, consider the tail integral:
\[
T_c \coloneqq \int_{[0,1] \setminus I_c} q(\hat{p})\,\exp\!\big(V_p(\hat{p})/c\big)\,d\hat{p}.
\]
Splitting into $\{|\delta| \in [c^{1/3}, \eta]\}$ and $\{|\delta| > \eta\}$: on the first region, $V_p(\hat{p}) \leq V_p(p) - \frac{1}{2}G''(p)\delta^2 + M|\delta|^3/6 \leq V_p(p) - \frac{1}{4}G''(p)\delta^2$ for $c$ small enough (since $|\delta| \leq \eta$ and $M|\delta|/3 \leq G''(p)/2$ when $\eta \leq 3G''(p)/(2M)$). On the second region, $V_p(\hat{p}) \leq V_p(p) - r$. Therefore:
\[
T_c \leq \bar{q}\!\left[\int_{|\delta| \geq c^{1/3}} \exp\!\Big(-\frac{G''(p)}{4c}\delta^2\Big)\,d\delta + \exp(-r/c)\right],
\]
where $\bar{q} = \sup_{[0,1]} q$. The Gaussian tail integral satisfies $\int_{|\delta| \geq c^{1/3}} \exp(-a\delta^2/2)\,d\delta \leq 2\exp(-a c^{2/3}/2)/\sqrt{a}$ by Mill's inequality. Since $a = G''(p)/(4c)$, the exponent is $-G''(p)c^{-1/3}/8$, which diverges as $c \to 0$. Both terms are therefore $o(c^N)$ for every $N > 0$.

Meanwhile, the mass on $I_c$ satisfies:
\[
\int_{I_c} q(\hat{p})\,\exp(V_p(\hat{p})/c)\,d\hat{p} \geq \underline{q}\,\exp(V_p(p)/c)\int_{-\sigma}^{\sigma}\exp(-a\delta^2)\,d\delta,
\]
where $\underline{q} = \inf_{I_c} q > 0$ for small $c$, and $\sigma = c^{1/3} \gg \sqrt{c/G''(p)}$. The inner integral is $\sqrt{\pi/a}\,(1 - o(c^N))$. Thus $T_c / \int_{I_c} = o(c^N)$ for every $N$, and all moments of $\pi^*(\cdot \mid p)$ may be computed on $I_c$ up to superpolynomially small error.

\medskip
\textit{Step 3: Moment computation on $I_c$.}
On $I_c$, where $|\delta| \leq c^{1/3}$, the remainder satisfies $|E(\delta)| \leq M c/6$ (since $|\delta|^3 \leq c$). Write:
\[
\frac{V_p(p+\delta)}{c} = \frac{V_p(p)}{c} - a\delta^2 + \frac{E(\delta)}{c}.
\]
Since $|E(\delta)/c| \leq M|\delta|^3/(6c)$ and $|\delta| \leq c^{1/3}$, we have $|E(\delta)/c| \leq M/(6c^0) = M/6$, but more precisely, for typical $\delta \sim \sqrt{c}$: $|E(\delta)/c| = O(c^{1/2})$. To make this rigorous, write:
\[
\exp\!\big(E(\delta)/c\big) = 1 + E(\delta)/c + O\big((E(\delta)/c)^2\big).
\]
On $I_c$, the remainder $E(\delta)/c$ admits the bound $|E(\delta)/c| \leq M|\delta|^3/(6c)$. Under the Gaussian weight $e^{-a\delta^2}$, the moments of $|\delta|^k$ are $O(c^{k/2})$, so $\langle |E(\delta)/c| \rangle_{a} = O(c^{1/2})$ and $\langle (E(\delta)/c)^2 \rangle_a = O(c)$.

The conditional density on $I_c$ is therefore:
\begin{equation}\label{eq:gaussian-approx}
\pi^*(\hat{p} \mid p) \propto q(p+\delta)\,\big(1 + E(\delta)/c + O(c)\big)\,\exp(-a\delta^2).
\end{equation}
Now expand $q$ around $p$. By assumption (A3), $q(p + \delta) = q(p)\big[1 + \beta\,\delta + O(\delta^2)\big]$ where $\beta = q'(p)/q(p)$. Combining with the remainder expansion:
\[
\pi^*(\hat{p} \mid p) \propto \big(1 + \beta\,\delta + O(\delta^2) + O(|\delta|^3/c)\big)\,\exp(-a\delta^2).
\]
We compute moments term by term, using the standard Gaussian integral identities on $\mathbb{R}$ (the extension from $I_c$ to $\mathbb{R}$ introduces only superpolynomially small error by Step~2):
\[
\int_{-\infty}^{\infty} e^{-a\delta^2}\,d\delta = \sqrt{\frac{\pi}{a}}, \quad \int_{-\infty}^{\infty} \delta^2\,e^{-a\delta^2}\,d\delta = \frac{1}{2a}\sqrt{\frac{\pi}{a}}, \quad \int_{-\infty}^{\infty} \delta\,e^{-a\delta^2}\,d\delta = 0,
\]
the last by antisymmetry. All odd-power integrals $\int \delta^{2k+1} e^{-a\delta^2}\,d\delta = 0$.

\textit{Mean.} The normalizing constant is $Z = \sqrt{\pi/a}\,\big(1 + O(c)\big)$. The first moment integral gives:
\[
\int \delta\,(1 + \beta\delta)\,e^{-a\delta^2}\,d\delta = \beta \cdot \frac{1}{2a}\sqrt{\frac{\pi}{a}},
\]
where the $\int \delta\,e^{-a\delta^2}\,d\delta = 0$ term vanishes. The $O(\delta^2)$ and $O(|\delta|^3/c)$ perturbations contribute $O(c^{3/2})$ to the mean (the $O(\delta^2)$ term yields an odd integral $\int \delta \cdot \delta^2 \,e^{-a\delta^2}\,d\delta = 0$; the cross-terms with $E(\delta)/c$ contribute at most $O(c^{1/2}) \cdot O(c) = O(c^{3/2})$ by Cauchy--Schwarz). Therefore:
\[
\mathbb{E}[\delta] = \frac{\beta}{2a} + O(c^{3/2}) = \frac{q'(p)}{q(p)} \cdot \frac{c}{G''(p)} + O(c^{3/2}).
\]

\textit{Second moment.} Similarly:
\[
\int \delta^2\,(1 + \beta\delta)\,e^{-a\delta^2}\,d\delta = \frac{1}{2a}\sqrt{\frac{\pi}{a}} + 0,
\]
where the $\beta\int \delta^3 e^{-a\delta^2}\,d\delta = 0$ by antisymmetry. The perturbation terms contribute $O(c^{3/2})$ to $\langle \delta^2 \rangle$ by the same reasoning. Thus $\langle \delta^2 \rangle = 1/(2a) + O(c^{3/2}) = c/G''(p) + O(c^{3/2})$.

\textit{Variance.} Combining:
\[
\operatorname{Var}(\delta) = \langle \delta^2 \rangle - \langle \delta \rangle^2 = \frac{c}{G''(p)} + O(c^{3/2}) - O(c^2) = \frac{c}{G''(p)} + O(c^{3/2}).
\]

\textit{Total variation bound.} Let $\varphi_\sigma$ denote the density of $N(0, \sigma^2)$ with $\sigma^2 = c/G''(p)$. On $I_c$, the density ratio $\pi^*(p + \delta \mid p)/\varphi_\sigma(\delta)$ equals $1 + \beta\delta + O(\delta^2) + O(|\delta|^3/c)$ times a normalizing correction of $1 + O(c)$. Under $\varphi_\sigma$, the expected absolute perturbation is $\langle |\beta\delta| \rangle_{\varphi_\sigma} = |\beta|\sqrt{2\sigma^2/\pi} = O(\sqrt{c})$. By Scheff\'e's identity, $d_{\mathrm{TV}}(\pi^*, \varphi_\sigma) = \frac{1}{2}\int |\pi^* - \varphi_\sigma| \leq O(\sqrt{c})$.
\end{proof}

\begin{remark}
Assumptions (A1)--(A2) are satisfied by all commonly used proper scoring rules. For the log score, $G(p) = -p\ln p - (1-p)\ln(1-p)$ gives $G''(p) = 1/[p(1-p)]$ and $G''' = (2p-1)/[p(1-p)]^2$, both bounded on any compact subset of $(0,1)$. For the Brier score, $G(p) = p(1-p)$ gives $G''(p) = 2$ and $G''' = 0$. Assumption (A3) is a mild regularity condition on the fixed-point marginal $q$, which inherits smoothness from the Gibbs kernel.
\end{remark}

The key consequence is:
\begin{equation}\label{eq:ri-gaussian}
\hat{p} - p \;\sim\; N\!\left(0,\; \frac{c}{G''(p)}\right) + O_{\mathrm{TV}}(\sqrt{c}) \quad \text{as } c \to 0^+.
\end{equation}
...
The pointwise optimization yields an optimal curvature function $g^*(p) = G''^*_{\mathrm{RI}}(p)$, but we must verify that a strictly proper scoring rule with this curvature actually exists. Specifically, we need to show that integrating $g^*$ twice and imposing the boundary conditions produces a valid entropy function $G^*$.

By the Green's function representation~\eqref{eq:greens}, define
\[
G^*(p) = \int_0^1 K(p,t)\,g^*(t)\,dt,
\]
where $K$ is the kernel~\eqref{eq:kernel}. This integral is well-defined and finite for all $p \in [0,1]$ provided $g^*$ is integrable on $(0,1)$, which holds whenever $|H(p)|\,f(p)/\phi(p)$ is bounded (guaranteed by $\phi(p) > 0$ on $(0,1)$ and $|H(p)|\,f(p)$ bounded).
...
\item \textbf{Boundary conditions.} Any $G$ constructed via~\eqref{eq:greens} automatically satisfies the boundary conditions $G(0) + G'(0) = 0$ and $G(1) - G'(1) = 0$. To see this: $G^*(0) = \int_0^1 K(0,t)\,g^*(t)\,dt = 0$ since $K(0,t) = 0$ for all $t$. Differentiating~\eqref{eq:greens} gives $(G^*)'(0) = -\int_0^1 t\,g^*(t)\,dt$, and from the double integration $G^*(0) = \int_0^1 t\,g^*(t)\,dt$, so $G^*(0) + (G^*)'(0) = 0$. The second boundary condition follows analogously.

\item \textbf{Limited liability.} Since the boundary conditions bind with equality, the reduced limited liability constraints hold. The standard representation gives $\min_{\hat{p}} S(\hat{p},1) = G^*(0) + (G^*)'(0) = 0 \geq 0$ and $\min_{\hat{p}} S(\hat{p},0) = G^*(1) - (G^*)'(1) = 0 \geq 0$. Therefore $S(\hat{p},x) \geq 0$ for all $\hat{p} \in (0,1)$ and $x \in \{0,1\}$.
...
The scoring rule corresponding to $G^*$ is then given by the standard representation:
\[
S^*(\hat{p},1) = G^*(\hat{p}) + (1-\hat{p})\,(G^*)'(\hat{p}), \qquad S^*(\hat{p},0) = G^*(\hat{p}) - \hat{p}\,(G^*)'(\hat{p}).
\]
This is a strictly proper scoring rule satisfying limited liability, confirming that the optimal curvature~\eqref{eq:ri-optimal-curvature} is attained by an admissible scoring rule.

\begin{remark}[Uniqueness]\label{rem:uniqueness}
The optimal scoring rule $S^*$ is unique within the class of limited-liability strictly proper scoring rules. The objective~\eqref{eq:ri-objective} is strictly concave in $g(p)$ at each $p$ (since $h''(g) < 0$), so the pointwise maximizer $g^*$ is unique. The Green's function map $g \mapsto G$ is linear and injective, and the two boundary conditions pin down the affine degrees of freedom that are otherwise free for proper scoring rules. There is therefore no residual indeterminacy.
\end{remark}

\begin{remark}[The optimal rule is generically novel]\label{rem:novel}
The optimal curvature $g^*(p) \propto \sqrt{|H(p)|\,f(p)/\phi(p)}$ depends on three problem-specific functions and does not, in general, belong to any standard parametric family of scoring rules. For instance, the power family $G''(p) \propto [p(1-p)]^{\beta-2}$ (which includes the Brier score at $\beta = 2$ and the logarithmic score as $\beta \to 1$) requires $|H(p)|\,f(p)/\phi(p) \propto [p(1-p)]^{2\beta-4}$, a condition that generically fails. Even in the simplest counterexample ($u(d,x) = -(d-x)^2$, $f \equiv 1$), the optimal curvature $g^*(p) \propto (p^2 - p + 1)^{-1/2}$ is not of this form. The optimal scoring rule is thus tailored to the specific decision problem and prior, rather than belonging to the standard toolkit of off-the-shelf rules.
\end{remark}


\section{Tail probability of large errors}\label{sec:ri-tail}

A major advantage of the rational inattention model is that the Gibbs distribution provides exponential bounds on the probability of large reporting errors, with the exponential rate determined exactly by the scoring rule's regret structure.
...
The tail probability of a reporting error exceeding $\Delta > 0$ is obtained by integrating the Gibbs distribution~\eqref{eq:gibbs-scoring} over the error region:
\[
\Pr(|\hat{p} - p| > \Delta \mid p) = \int_{|\hat{p}-p|>\Delta} \pi^*(\hat{p} \mid p)\,d\hat{p} = \int_{|\hat{p}-p|>\Delta} \frac{q(\hat{p})\,\exp(V_p(\hat{p})/c)}{Z(p)}\,d\hat{p}.
\]
Since the normalizing constant is $Z(p) = \int_0^1 q(\hat{p})\,\exp(V_p(\hat{p})/c)\,d\hat{p}$, this becomes:
...
\]
The constant $\frac{\bar{q}}{2\epsilon\,\underline{q}(\epsilon)}$ depends on $q$ and $\epsilon$ but not on $c$. Since $R^*(\epsilon, p) \to 0$ as $\epsilon \to 0$ while $R^*(\Delta,p) > 0$, we can choose $\epsilon$ small enough that $R^*(\Delta,p) - R^*(\epsilon,p) > 0$. The bound then has the form $C \cdot \exp(-(\text{positive constant})/c)$ where $C$ does not depend on $c$, establishing the exponential decay~\eqref{eq:tail-bound}.
\end{proof}

The bound~\eqref{eq:tail-bound} has several notable features:

\begin{itemize}
\item \textbf{Exponential rate from the Gibbs structure.} The rate $R^*(\Delta,p)/c$ is a direct consequence of the Gibbs form~\eqref{eq:gibbs}---not of a Gaussian approximation. The exponential decay holds for the exact optimal reporting distribution at any $c$; only the multiplicative constant $C$ depends on the endogenous marginal $q$.
...
For small $\Delta$, consider a report $\hat{p}$ with $|\hat{p} - p| = \Delta$. The regret is well approximated by the second-order Taylor expansion of $V_p$ around $p$:
\begin{equation}\label{eq:regret-quadratic}
R_p(\hat{p}) = V_p(p) - V_p(\hat{p}) \approx \frac{1}{2}\,G''(p)\,(\hat{p}-p)^2 = \frac{1}{2}\,G''(p)\,\Delta^2,
\end{equation}
where the first-order term vanishes because $V_p'(p) = 0$ by properness.
...
\end{equation}
Note that the exponent $G''(p)\,\Delta^2/(2c)$ is exactly $\Delta^2/(2\sigma^2_p)$ where $\sigma^2_p = c/G''(p)$ is the Gaussian approximation variance from~\eqref{eq:ri-gaussian}. So the quadratic regret approximation recovers the same tail decay as the Gaussian approximation~\eqref{eq:ri-gaussian}. The point of the Gibbs framework is that exponential tail decay holds more generally: it follows from the structure of~\eqref{eq:gibbs-scoring} and properness alone, without relying on the Gaussian approximation.
...
\subsection{The subcontracting setting}

Suppose the principal is herself a forecaster, scored by a strictly proper scoring rule $S_0$ with entropy function $G_0$. She delegates the estimation of $p$ to a sub-agent, whom she pays according to a scoring rule $S$ of her choice. Her utility function is $u(d,x) = S_0(d,x)$, where $d$ is the report she submits to the meta-principal. Since $S_0$ is strictly proper, the principal's optimal decision given the agent's report is simply $d^*(\hat{p}) = \hat{p}$, so $(d^*)'(\hat{p}) = 1$.

The principal's sensitivity~\eqref{eq:H-def} then simplifies to $H(p) = u''(d^*(p),p) = -G_0''(p)$, so $|H(p)| = G_0''(p)$.

A natural benchmark is the \emph{pass-through rule}: the principal assigns the sub-agent a scaled version of her own rule $S_0$. Scaling $S_0$ by $a > 0$ scales the curvature by $a$, so the pass-through rule has $g(p) = a\,G_0''(p)$ for some $a > 0$ chosen optimally. We compare the principal's net utility under this rule against the pointwise-optimal curvature~\eqref{eq:ri-optimal-curvature}.

\subsection{Optimal rule value}

The principal's net utility under curvature profile $g$ is given by the objective~\eqref{eq:ri-objective}:
\[
V(g) = \int_0^1 \left[-\frac{c\,|H(p)|\,f(p)}{2\,g(p)} - \alpha\,\phi(p)\,g(p)\right] dp.
\]
In the subcontracting setting $|H(p)| = G_0''(p)$, so substituting the pointwise-optimal curvature $g^*(p) = \sqrt{c\,G_0''(p)\,f(p)/(2\alpha\,\phi(p))}$ from~\eqref{eq:ri-optimal-curvature} into the integrand (writing $A = c\,G_0''\,f/2$ and $C = \alpha\phi$):
\[
-\frac{A}{g^*} - C\,g^* = -\frac{A}{\sqrt{A/C}} - C\sqrt{A/C} = -2\sqrt{AC}.
\]
Therefore:
\begin{equation}\label{eq:ri-V-opt}
V^*_{\mathrm{RI}} = -2\int_0^1 \sqrt{\frac{c\,G_0''(p)\,f(p)}{2} \cdot \alpha\,\phi(p)} \; dp = -\sqrt{2c\alpha} \int_0^1 \sqrt{G_0''(p)\,f(p)\,\phi(p)} \; dp.
\end{equation}

\subsection{Pass-through rule value}

Under the pass-through rule, $g(p) = a\,G_0''(p)$. Substituting into the objective:
\[
V(a) = \int_0^1 \left[-\frac{c\,G_0''\,f}{2a\,G_0''} - \alpha\,\phi\,a\,G_0''\right]dp = -\frac{c}{2a}\underbrace{\int_0^1 f(p)\,dp}_{= 1} \;-\; \alpha\,a\int_0^1 \phi(p)\,G_0''(p)\,dp.
\]
(Here $|H| = G_0''$ cancels in the first term.) Differentiating with respect to $a$ and setting to zero:
\[
V'(a) = \frac{c}{2a^2} - \alpha\int_0^1 \phi\,G_0''\,dp = 0 \qquad\Longrightarrow\qquad a^* = \sqrt{\frac{c}{2\alpha\int_0^1 \phi\,G_0''\,dp}}.
\]
Substituting back:
\[
V_{\mathrm{pass}} = V(a^*) = -\sqrt{2c\alpha\int_0^1 \phi(p)\,G_0''(p)\,dp}.
\]

\subsection{Efficiency ratio}\label{sec:efficiency}

Since both $V^*_{\mathrm{RI}}$ and $V_{\mathrm{pass}}$ are negative, with $V^*_{\mathrm{RI}}$ closer to zero, define the ratio
\begin{equation}\label{eq:ri-efficiency}
\eta_{\mathrm{RI}} = \frac{V^*_{\mathrm{RI}}}{V_{\mathrm{pass}}} = \frac{\displaystyle\int_0^1 \sqrt{G_0''(p)\,f(p)\,\phi(p)}\;dp}{\displaystyle\sqrt{\int_0^1 \phi(p)\,G_0''(p)\,dp}},
\end{equation}
where we used $\int f = 1$. We now show that $\eta \in (0,1]$. Positivity is immediate since $G_0'', f, \phi > 0$. For the upper bound, we need:
\[
\left(\int_0^1 \sqrt{G_0''\,f\,\phi}\;dp\right)^2 \leq \int_0^1 G_0''\,\phi\,dp.
\]
Write $\int \sqrt{G_0''\,f\,\phi}\,dp = \int \sqrt{f} \cdot \sqrt{G_0''\,\phi}\,dp$ and apply the Cauchy--Schwarz inequality with $u = \sqrt{f}$ and $v = \sqrt{G_0''\,\phi}$:
\[
\int \sqrt{G_0''\,f\,\phi}\,dp \leq \sqrt{\int f\,dp} \cdot \sqrt{\int G_0''\,\phi\,dp} = \sqrt{\int G_0''\,\phi\,dp}.
\]
Squaring yields the desired inequality, so $\eta \leq 1$. Equality holds if and only if $\sqrt{f} \propto \sqrt{G_0''\,\phi}$, i.e., $G_0''(p)\,\phi(p)/f(p) = \text{const}$, which is exactly the condition~\eqref{eq:ri-Su-condition} for $S_u$ to be optimal. The fractional improvement from using the optimal rule is $1 - \eta$.

The efficiency ratio $\eta$ is \textbf{parameter-free}: it depends only on the shape functions $G_0''$, $f$, and $\phi$, not on $c$ or $\alpha$. The percentage gain from optimization is the same regardless of the attention cost or the cost of payments.


\section{Extension to continuous outcomes}\label{sec:continuous}

The preceding analysis assumes a binary event $X \in \{0,1\}$, where the agent reports a single probability $\hat{p} = \Pr(X=1)$. Many forecasting problems involve continuous outcomes: predicting a temperature, a stock return, or an unemployment rate. We now extend the rational inattention framework to this setting, focusing on the elicitation of the mean. The main finding is that the entire framework---Gibbs solution, Gaussian approximation, variance formula, optimal curvature, and curvature allocation principle---carries over with essentially no change. The binary case is a special case of the continuous one.

\subsection{Consistent scoring functions for the mean}\label{sec:mean-scoring}

Let $Y$ be a continuous random variable with support $[a,b]$ and mean $\mu = \mathbb{E}[Y]$, where $\mu \in [a,b]$. A principal wishes to elicit the mean from an agent. The agent reports $\hat{\mu} \in [a,b]$ and receives a payment $S(\hat{\mu}, y)$ when the outcome $y$ is realized. The scoring function $S$ is \emph{strictly consistent for the mean} if the agent's expected score $V_\mu(\hat{\mu}) \equiv \mathbb{E}[S(\hat{\mu}, Y) \mid \mu]$ is uniquely maximized at $\hat{\mu} = \mu$ for every distribution on $[a,b]$ with mean $\mu$.

Savage (1971) and Gneiting (2011) characterize such scoring functions. A scoring function $S(\hat{\mu}, y)$ is strictly consistent for the mean (over all distributions on $[a,b]$) if and only if it takes the form
\begin{equation}\label{eq:mean-representation}
S(\hat{\mu}, y) = G(\hat{\mu}) + G'(\hat{\mu})(y - \hat{\mu}) + h(y),
\end{equation}
where $G: [a,b] \to \mathbb{R}$ is a strictly convex function (the \emph{entropy function}), $G'$ denotes its derivative, and $h: [a,b] \to \mathbb{R}$ is an arbitrary function of the outcome alone. The expected score is then:
\begin{equation}\label{eq:mean-expected-score}
V_\mu(\hat{\mu}) = G(\hat{\mu}) + G'(\hat{\mu})(\mu - \hat{\mu}) + \mathbb{E}[h(Y)].
\end{equation}
Since $\mathbb{E}[h(Y)]$ does not depend on the report $\hat{\mu}$, it is irrelevant for the agent's optimization and can be absorbed into a constant. The remaining terms $G(\hat{\mu}) + G'(\hat{\mu})(\mu - \hat{\mu})$ are \textbf{identical in form} to the binary case, with $\mu$ replacing $p$.

Differentiating~\eqref{eq:mean-expected-score} with respect to $\hat{\mu}$:
\begin{equation}\label{eq:mean-Vprime}
V_\mu'(\hat{\mu}) = G''(\hat{\mu})(\mu - \hat{\mu}),
\end{equation}
which confirms $V_\mu'(\mu) = 0$ and
\begin{equation}\label{eq:mean-Vpp}
V_\mu''(\mu) = -G''(\mu).
\end{equation}
The expected score has a strict maximum at $\hat{\mu} = \mu$ with curvature determined by $G''(\mu) > 0$, exactly as in the binary case.

\begin{remark}[The binary case as a special case]\label{rem:binary-special}
When $Y \in \{0,1\}$ is binary with $\Pr(Y=1) = p$, the mean is $\mu = p$. A scoring function $S(\hat{\mu}, y)$ consistent for the mean of a binary variable is exactly a proper scoring rule $S(\hat{p}, x)$. Representation~\eqref{eq:mean-representation} reduces to:
\[
S(\hat{p}, 1) = G(\hat{p}) + G'(\hat{p})(1 - \hat{p}) + h(1), \qquad S(\hat{p}, 0) = G(\hat{p}) - G'(\hat{p})\hat{p} + h(0),
\]
which is the standard entropy function representation of proper scoring rules (with the constants $h(1)$ and $h(0)$ absorbed by convention). The entire binary framework is a special case of mean elicitation with $[a,b] = [0,1]$ and $Y \in \{0,1\}$.
\end{remark}

\begin{remark}[Examples of consistent scoring functions for the mean]
The \emph{squared error} score $S(\hat{\mu}, y) = -(y - \hat{\mu})^2$ corresponds to $G(\hat{\mu}) = \hat{\mu}^2$, $h(y) = -y^2$, giving constant curvature $G''(\hat{\mu}) = 2$---the direct analogue of the Brier score.
\end{remark}


\subsection{Transfer of results to mean elicitation}

Since the expected score~\eqref{eq:mean-expected-score} has the same functional form $G(\hat{\mu}) + G'(\hat{\mu})(\mu - \hat{\mu})$ as in the binary case, every result from the preceding sections carries over with $\mu$ replacing $p$ and $[a,b]$ replacing $[0,1]$: the Gibbs solution (Proposition~\ref{prop:gibbs}), the Gaussian approximation with variance $c/G''(\mu)$, the exponential tail bound (Proposition~\ref{prop:tail-bound}), and the optimal curvature formula~\eqref{eq:ri-optimal-curvature}. The efficiency analysis of the pass-through rule likewise applies verbatim with $G_0''(\mu)$ replacing $G_0''(p)$.

The only changes are mechanical. Under limited liability with $h(y) = 0$, the boundary conditions become:
\begin{equation}\label{eq:bc-general}
G(a) + G'(a)(b - a) = 0, \qquad G(b) - G'(b)(b - a) = 0,
\end{equation}
which for $[a,b] = [0,1]$ reduce to the binary conditions $G(0) + G'(0) = 0$ and $G(1) - G'(1) = 0$. The Green's function for integrating $G''(\mu) = g(\mu)$ subject to~\eqref{eq:bc-general} is:
\begin{equation}\label{eq:kernel-general}
K_{[a,b]}(\mu, t) = \frac{1}{b-a}\begin{cases} (\mu - a)(b - t), & t \leq \mu, \\ (t - a)(b - \mu), & t > \mu, \end{cases}
\end{equation}
so that $G(\mu) = \int_a^b K_{[a,b]}(\mu, t)\, g(t)\, dt$, generalizing~\eqref{eq:greens}. The optimal curvature is then:
\begin{equation}\label{eq:optimal-curvature-mean}
G''(\mu)^* = \sqrt{\frac{c\,|H(\mu)|\,f(\mu)}{2\alpha\,\phi(\mu)}},
\end{equation}
where $\phi(\mu) = \int_a^b K_{[a,b]}(t, \mu)\, f(t)\, dt$, exactly as before.


\subsection{Beyond the mean: quantile elicitation}\label{sec:quantile}

The mean is not the only functional of interest. In risk management and distributional forecasting, quantiles are often the primary target. The rational inattention framework applies, but the structure differs in important ways.

Let $F$ denote the CDF of $Y$ and $f_Y$ its density. For the $\tau$-quantile $q_\tau$ (where $F(q_\tau) = \tau$), Gneiting (2011) shows that for any strictly increasing function $g$, the negatively oriented scoring function (loss)
\[
L(\hat{q}, y) = (\mathbf{1}\{y \leq \hat{q}\} - \tau)\big(g(\hat{q}) - g(y)\big)
\]
is strictly consistent: the expected loss $\mathbb{E}[L(\hat{q}, Y)]$ is minimized at $\hat{q} = q_\tau$. To align with the rest of the paper (where the agent maximizes a score), we work with the negated score $S(\hat{q}, y) = -L(\hat{q}, y)$. The expected score is:
\[
V_{q_\tau}(\hat{q}) = -\mathbb{E}\big[(\mathbf{1}\{Y \leq \hat{q}\} - \tau)(g(\hat{q}) - g(Y))\big],
\]
Writing out the expectation and splitting by whether $y \leq \hat{q}$:
\[
V(\hat{q}) = -\int_a^{\hat{q}} (g(\hat{q}) - g(y))\,f_Y(y)\,dy + \tau\big(g(\hat{q}) - \mathbb{E}[g(Y)]\big).
\]
Differentiating with respect to $\hat{q}$ using the Leibniz rule $\frac{d}{d\hat{q}} \int_a^{\hat{q}} h(\hat{q}, y)\,dy = h(\hat{q}, \hat{q}) + \int_a^{\hat{q}} \frac{\partial}{\partial \hat{q}} h(\hat{q}, y)\,dy$ on the first term:
\[
\frac{d}{d\hat{q}} \int_a^{\hat{q}} (g(\hat{q}) - g(y))\,f_Y(y)\,dy = \underbrace{(g(\hat{q}) - g(\hat{q}))\,f_Y(\hat{q})}_{= 0} + \int_a^{\hat{q}} g'(\hat{q})\,f_Y(y)\,dy = g'(\hat{q})\,F(\hat{q}).
\]
Combining with the second term:
\[
V'(\hat{q}) = -g'(\hat{q})\,F(\hat{q}) + \tau\,g'(\hat{q}) = -(F(\hat{q}) - \tau)\,g'(\hat{q}),
\]
confirming $V'(q_\tau) = 0$ since $F(q_\tau) = \tau$. Differentiating again:
\[
V''(\hat{q}) = -f_Y(\hat{q})\,g'(\hat{q}) - (F(\hat{q}) - \tau)\,g''(\hat{q}).
\]
At $\hat{q} = q_\tau$ the second term vanishes, giving:
\begin{equation}\label{eq:quantile-curvature}
V''(q_\tau) = -f_Y(q_\tau)\,g'(q_\tau),
\end{equation}
which is strictly negative (confirming that the truth is a strict maximum). The reporting variance under rational inattention for quantile forecasts is therefore:
\begin{equation}\label{eq:quantile-variance}
\operatorname{Var}(\hat{q} - q_\tau \mid q_\tau) \approx \frac{c}{f_Y(q_\tau)\,g'(q_\tau)}.
\end{equation}

This differs from the mean case in two respects. First, the variance depends on the \emph{density at the quantile} $f_Y(q_\tau)$---quantiles in regions where the density is high are easier to estimate precisely, because even a small shift in the report crosses many probability mass units. Second, the principal chooses the increasing function $g$, with $g'$ playing the role of curvature. Increasing $g'$ at a given quantile value sharpens incentives there, analogously to increasing $G''$ in the mean case.

The principal's optimization is more complex than in the mean case: the expected payment $\mathbb{E}[S(q_\tau, Y)]$ depends on $g$ through the full distribution of $Y$ (not just the quantile being elicited), so the separability that made the mean case tractable does not carry over directly. We leave the full analysis of optimal scoring functions for quantile elicitation as an open direction for future work.


\section{Approximation by parametric families}\label{sec:approximation}

The optimal scoring rule $S^*$ is tailored to the specific decision problem $u$, prior $f$, and cost parameters $c$ and $\alpha$, and does not generally belong to any standard parametric family (Remark~\ref{rem:novel}). In practice, a forecasting platform may prefer to deploy a rule from a simple parametric class. This section quantifies the welfare cost of such restrictions and identifies when simple rules suffice.

\subsection{Best constant curvature (scaled Brier)}

The simplest restriction is $g(p) = g_0$ (constant curvature), corresponding to a scaled Brier score. Substituting into the objective~\eqref{eq:ri-objective}:
\[
V(g_0) = -\frac{c\,\bar{H}}{2\,g_0} - \alpha\,\bar{\phi}\,g_0,
\]
where $\bar{H} = \int_0^1 |H(p)|\,f(p)\,dp$ and $\bar{\phi} = \int_0^1 \phi(p)\,dp$. Optimizing over $g_0$ yields:
\begin{equation}\label{eq:best-brier}
g_0^* = \sqrt{\frac{c\,\bar{H}}{2\alpha\,\bar{\phi}}}, \qquad V^*_{\mathrm{Brier}} = -2\sqrt{\frac{c\,\alpha\,\bar{H}\,\bar{\phi}}{2}}.
\end{equation}

Since both values are negative (with $V^*$ closer to zero), we define the efficiency ratio as in Section~\ref{sec:efficiency}:
\begin{equation}\label{eq:brier-efficiency}
\eta_{\mathrm{Brier}} = \frac{V^*}{V^*_{\mathrm{Brier}}} = \frac{\int_0^1 \sqrt{|H(p)|\,f(p)\,\phi(p)}\,dp}{\sqrt{\bar{H}\,\bar{\phi}}}.
\end{equation}
By Cauchy--Schwarz (with $u = \sqrt{|H|f}$ and $v = \sqrt{\phi}$), $\eta_{\mathrm{Brier}} \in (0,1]$, with equality if and only if $|H(p)|\,f(p)/\phi(p)$ is constant in $p$---exactly the condition for the Brier score to be optimal. This ratio is parameter-free: it depends on the decision problem and prior, but not on $c$ or $\alpha$.

\subsection{Best power-family rule}\label{sec:power-approx}

A richer one-parameter family is the \emph{power class}: $g(p) = A\,[p(1-p)]^{-\gamma}$ for $\gamma \in [0,1)$ and $A > 0$. This nests the Brier score ($\gamma = 0$) and approaches the logarithmic score as $\gamma \to 1$. For each $\gamma$, substituting into~\eqref{eq:ri-objective} and optimizing over $A$ gives:
\[
A^*(\gamma) = \sqrt{\frac{c\int_0^1 |H(p)|\,f(p)\,[p(1-p)]^{\gamma}\,dp}{2\alpha\int_0^1 \phi(p)\,[p(1-p)]^{-\gamma}\,dp}},
\]
and the corresponding net utility:
\begin{equation}\label{eq:power-value}
V^*_{\mathrm{power}}(\gamma) = -2\sqrt{\frac{c\,\alpha}{2}} \sqrt{\int_0^1 |H(p)|\,f(p)\,[p(1-p)]^{\gamma}\,dp \;\cdot\; \int_0^1 \phi(p)\,[p(1-p)]^{-\gamma}\,dp}.
\end{equation}
The optimal $\gamma^*$ minimizes $V^*_{\mathrm{power}}(\gamma)$ in magnitude, i.e., minimizes the product of the two integrals. This is a one-dimensional problem that can be solved numerically.

\subsection{Examples}\label{sec:approx-examples}

We evaluate the efficiency ratios for the counterexample $u(d,x) = -(d-x)^2$ (so $|H(p)| = 2$) under different priors.

\paragraph{Uniform prior.} With $f \equiv 1$, $\phi(p) = (p^2 - p + 1)/2$. The averages are $\bar{H} = 2$ and $\bar{\phi} = 5/12$. Numerically, $\int_0^1 \sqrt{p^2-p+1}\,dp \approx 0.9120$, giving:
\[
\eta_{\mathrm{Brier}} = \frac{0.9120}{\sqrt{5/6}} \approx 0.999.
\]
The welfare loss from using the Brier score is approximately $0.1\%$---negligible. This is because $\phi(p) = (p^2-p+1)/2$ varies only between $3/8$ and $1/2$ under the uniform prior, so the ratio $|H|\,f/\phi$ is nearly constant and the Brier score is nearly optimal. The best power-family rule ($\gamma^* \approx 0$) offers no meaningful improvement.

\paragraph{Concentrated priors.} The approximation gap grows substantially when the prior concentrates probability mass unevenly. With a $\mathrm{Beta}(2,2)$ prior ($f(p) = 6p(1-p)$, peaked at $1/2$), the Brier efficiency drops to $\eta_{\mathrm{Brier}} \approx 0.95$, a $5\%$ welfare loss. With a $\mathrm{Beta}(5,5)$ prior (more sharply peaked), $\eta_{\mathrm{Brier}} \approx 0.79$---a $21\%$ welfare loss. Asymmetric priors such as $\mathrm{Beta}(2,5)$ yield similar gaps ($\eta_{\mathrm{Brier}} \approx 0.73$).

The power family $g(p) = A[p(1-p)]^{-\gamma}$ provides little additional improvement in these cases: the best power rule achieves essentially the same efficiency as the Brier score. This is because the variation in $|H|\,f/\phi$ is driven by the interaction of the prior $f$ with the cost-of-curvature function $\phi(p) = \int K(t,p)\,f(t)\,dt$, which depends on the prior in a way that $[p(1-p)]^{-\gamma}$ cannot approximate. The notable exception is the $\mathrm{Beta}(1/2,1/2)$ prior, where $f(p) \propto [p(1-p)]^{-1/2}$ and the power family achieves $\eta_{\mathrm{power}} \approx 1.00$---the optimal curvature itself lies (approximately) in the power class.

\paragraph{When does the gap matter?} The efficiency ratio $\eta_{\mathrm{Brier}}$ is determined by the variation of $|H(p)|\,f(p)/\phi(p)$ across $p$. When this ratio is nearly constant---as with the uniform prior and symmetric loss---the Brier score is effectively optimal and the choice of scoring rule is inconsequential beyond properness. When the ratio varies substantially---due to concentrated or asymmetric priors, or non-constant $|H(p)|$ from asymmetric loss functions---the full optimal rule provides a meaningful welfare improvement that simple parametric families cannot capture.
