...
To resolve this, we explicitly constrain the agent's action space to the interval $[0,1]$. Because $S$ is a strictly proper scoring rule, the expected score $V_p(\hat{p})$ is strictly concave in $\hat{p}$. The addition of the linear noise term $\theta \hat{p}$ preserves this strict concavity.

For a strictly concave objective function maximized over a closed interval, if the unconstrained maximum falls outside the interval, the constrained maximum is the nearest boundary point. The agent's actual report is therefore the clipped value:
$$\hat{p}_c = \max(0, \min(1, \hat{p}_{raw}))$$

We now establish that the continuous Taylor approximation remains valid as a conservative upper bound for the principal's expected loss. Let the true probability be $p \in (0,1)$. Because $p$ lies within the valid interval, clipping a report that falls outside $[0,1]$ strictly moves the report closer to $p$. Mathematically:
$$|\hat{p}_c - p| \le |\hat{p}_{raw} - p|$$

Squaring both sides and taking the expectation over $\theta$, the expected squared error of the clipped report is strictly bounded above by the variance of the unconstrained report:
$$\mathbb{E}[(\hat{p}_c - p)^2] \le \mathbb{E}[(\hat{p}_{raw} - p)^2] \approx \frac{\sigma^2}{[G''(p)]^2}$$

The principal's loss relative to knowing $p$ exactly is approximated by:
$$L \approx -\frac{1}{2}H(p)(\hat{p} - p)^2$$

 Substituting the unconstrained variance into the expected loss formula yields a strict upper bound on the principal's true expected loss:
$$\mathbb{E}[L_c | p] \le -\frac{\sigma^2 H(p)}{2[G''(p)]^2}$$

Consequently, optimizing the scoring rule against the unconstrained analytical loss formula guarantees the principal at least that level of expected utility under the true, clipped error distribution.
...
\textbf{Status.} Sections 1--6 set up the model: we set up the principal-agent problem, define the decision-matched scoring rule $S_u$, prove that the scoring rule is proper, define the random utility model with Gaussian noise, transform the problem to log-odds to avoid the unbounded support problem, and define the principal's optimization problem. Section~\ref{sec:limited-liability} solves the optimal scoring rule under limited liability ($S \geq 0$), and we achieve a closed-form solution for optimal curvature.
...
\item The principal's problem becomes separable in $g$. This lets us optimize point-wise.
\item The optimal curvature is $|G''(p)|^* = \left(\sigma^2 |H(p)| f(p) / (\alpha |\phi(p)|)\right)^{1/3}$.
\item $S_u$ is optimal if and only if $|H(p)|^2 |\phi(p)| / f(p) = \text{const}$. This is not generally true for generic utility functions and priors.
\end{itemize}
